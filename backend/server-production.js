...
// Try to require database modules, fallback to in-memory if not available
let connectDB, Poll, Participant, ChatMessage;
let useDatabase = false;

try {
  connectDB = require('../database/connection');
  Poll = require('../database/models/Poll');
  Participant = require('../database/models/Participant');
  ChatMessage = require('../database/models/ChatMessage');
  useDatabase = true;
  console.log('✅ Database modules loaded successfully');
} catch (error) {
  console.log('⚠️  Database modules not found, using in-memory storage');
  useDatabase = false;
}
...
// In-memory data storage (fallback when database is not available)
let polls = new Map();
let pollHistory = [];
...
server.listen(PORT, () => {
  ...
  console.log(`💾 Database: ${useDatabase ? 'MongoDB Connected' : 'In-memory storage'}`);
});
